...
namespace uniform {
	...
	void Shader_Standard_Skybox_Uniforms::SetBaseColor(glm::vec3 _color) { m_baseColor = _color; }
	void Shader_Standard_Skybox_Uniforms::SetTextureScale(glm::vec2 _scale) { m_TextureTiling = _scale; }

	 void Shader_Standard_Skybox_Uniforms::SetColour(const glm::vec4& value, const std::string& name) {
	 ...
	 void Shader_Standard_Skybox_Uniforms::SetFloat(const float& value, const std::string& name) {
	 ...
	 void Shader_Standard_Skybox_Uniforms::SetInt(const int& value, const std::string& name) {
		std::cout << " could not find uniform with name " << name << " of type int " << std::endl;
	 }


	void Shader_Standard_Skybox_Uniforms::SetUniformMVP(glm::mat4& model, glm::mat4& view, glm::mat4& projection, ShaderProgram& _shader, Camera& _camera) {
		...
		_shader.setUniform("projMatrix", projection);
		_shader.setUniform("camPos", _camera.GetPosition());
	}

	void Shader_Standard_Skybox_Uniforms::SetUniformCustom(ShaderProgram& _shader) {
		...
		glBindTexture(GL_TEXTURE_CUBE_MAP, skybox.GetTexture()); // bind
