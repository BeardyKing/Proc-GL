#ifndef SHADER_TERRAIN_UNIFORMS_H
#define SHADER_TERRAIN_UNIFORMS_H
...
namespace uniform {
	class Shader_Terrain_Uniforms : public Shader_Uniforms
	{
	public:
		Shader_Terrain_Uniforms();
		~Shader_Terrain_Uniforms();
	...
	void Shader_Terrain_Uniforms::SetBaseColor(glm::vec3 _color) { m_baseColor = _color; }
	void Shader_Terrain_Uniforms::SetTextureScale(glm::vec2 _scale) { m_TextureTiling = _scale; }

	 void Shader_Terrain_Uniforms::SetColour(const glm::vec4& value, const std::string& name) {
	 ...
	 void Shader_Terrain_Uniforms::SetFloat(const float& value, const std::string& name) {
	 ...
	 void Shader_Terrain_Uniforms::SetInt(const int& value, const std::string& name) {
		std::cout << " could not find uniform with name " << name << " of type int " << std::endl;
	 }


	void Shader_Terrain_Uniforms::SetUniformMVP(glm::mat4& model, glm::mat4& view, glm::mat4& projection, ShaderProgram& _shader, Camera& _camera) {
	...
	void Shader_Terrain_Uniforms::SetUniformCustom(ShaderProgram& shader) {
	...
	void Shader_Terrain_Uniforms::OnImGuiRender() {
	...
	void Shader_Terrain_Uniforms::LoadTextures(Entity& _shader) {
